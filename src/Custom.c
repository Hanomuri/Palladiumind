...
static void PrintCustomSection( const struct CustomPage* customPage, bool rev ) {
...
static void PrintBookPageEntry( const struct BookEntry* bookEntry, const u_short type, bool rev )
{
...
void ReadCustomData(Mind* mind) {
  ...
  #define CUSTOM_PAGE_SCAN fscanf(readCustomFile, "%hhu %[^\n]s", &customPage.sectionType, customPage.name)
  for(; CUSTOM_PAGE_SCAN != EOF;) {
    printf("%d ", ++currentPage);
    if (mind->currentNavi == currentPage)
    {
      PrintCustomSection(&customPage, 1);
    }
    else
    {
      PrintCustomSection(&customPage, 0);
    }
  ...
  mind->maximumNavi = currentPage;
  if (mind->maximumNavi < mind->currentNavi)
  {
    mind->currentNavi = mind->maximumNavi;
  }
...
void ReadCustomPage(Mind* mind) {
  ...
  unsigned char type;
  fscanf(readCustomEntriesFile, "%hhu\n", &type);
  
  int currentEntry = 0;

  if(type & BOOK_PAGE) {
    BookEntry bookEntry = GenBookEntry();
    #define CUSTOM_ENTRY_SCAN fscanf(readCustomEntriesFile, "%hd%hhu%hhu %[^\n]s", &bookEntry.data, &bookEntry.year, &bookEntry.score, bookEntry.name)
    for(; CUSTOM_ENTRY_SCAN != EOF;) {
      fgetc(readCustomEntriesFile);
      fscanf(readCustomEntriesFile, "%[^\n]s", bookEntry.author);
      printf("%d ", ++currentEntry);
      if (mind->currentNavi == currentEntry)
      {
        PrintBookPageEntry(&bookEntry, type, 1);
      }
      else
      {
        PrintBookPageEntry(&bookEntry, type, 0);
      }
    }
    free(bookEntry.name);
    ...
    #define ENTRY_SCAN fscanf(readCustomEntriesFile, "%hd%hhu%hhu %[^\n]s", &entry.data, &entry.year, &entry.group, entry.name)
    for(; ENTRY_SCAN != EOF;) {
      printf("%d ", ++currentEntry);
      if (mind->currentNavi == currentEntry)
      {
...
void DisplayBoard(const char* filepath) {
  ...
  #define BOARD_SCAN fscanf(readBookFile, "%hd%hhu%hhu %[^\n]s", &entry.data, &entry.year, &entry.score, entry.name)
